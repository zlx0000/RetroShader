...
void main()
{
    ...
    vec3 pos = rndv3((gl_ModelViewMatrix * gl_Vertex).xyz);
    pos = rndv3((gbufferModelViewInverse * vec4(pos,1)).xyz);

    //Output position and fog to fragment shader.
    gl_Position = rndv4(gl_ProjectionMatrix * gbufferModelView * vec4(pos,1));

    //Output color to fragment shader.
    color = rndv4(vec4(gl_Color.rgb, gl_Color.a));
